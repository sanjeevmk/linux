			   ...
			   u64 devid, struct btrfs_fs_devices **fs_devices_ret)
{
	...
	if (!device) {
		...
		device->devid = devid;
		device->device_stats_valid = 0; //+smk
...
{
// -smk	struct btrfs_bio *bbio = bio->bi_private;
				...
				int reset_after_read)
{
	...
	if (!dev) {
		printk(KERN_WARNING
			...
			return -ENODEV;
	} /*+smk*/ else if (!dev->device_stats_valid) {
		printk(KERN_WARNING
				"btrfs: get device_stats failed, not yet valid\n");
		return -ENODEV;
	} /*+smk*/ else if (reset_after_read) {
		 if (stats->nr_items >= 1)
